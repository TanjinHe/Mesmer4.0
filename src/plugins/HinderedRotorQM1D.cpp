	...
	bool HinderedRotorQM1D::ReadParameters(gDensityOfStates* gdos, PersistPtr ppDOSC)
	{
		...
		if (pp) {
			...
			if (inertUseSine)
			{
				...
				m_kineticSinCoeff.resize(maxIndex/2+1);
				m_kineticSinCoeff[0] = 0;
				for (size_t	i(0); i < coefficients.size(); i++)
				{
					if (indicies[i] < (maxIndex/2+1))
					{
						m_kineticCosCoeff[indicies[i]] = coefficients[i];
					}
					else
					{
						m_kineticSinCoeff[indicies[i]-maxIndex/2] = coefficients[i];
					}
				}
				for (size_t i(0); i < maxIndex/2+1; i++)
				{
					printf("indix:\t%d\t%f\t%f\n",i,m_kineticCosCoeff[i],m_kineticSinCoeff[i]);
				}
